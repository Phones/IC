...
class cromossomo:
    ...
    def __lt__(self, other):
        return (self.custo < other.custo and self.soma_tam_caminhos <= other.soma_tam_caminhos) or (
                    self.custo <= other.custo and self.soma_tam_caminhos < other.soma_tam_caminhos)
    ...
    def __add__(self, other):
        while True:
            ...
            if f1 != None:
                ...
                if ((filho.custo < self.custo and filho.soma_tam_caminhos <= self.soma_tam_caminhos) or (
                        filho.custo <= self.custo and filho.soma_tam_caminhos < self.soma_tam_caminhos)) and (
                        (filho.custo <= other.custo and filho.soma_tam_caminhos < other.soma_tam_caminhos) or (
                        filho.custo < other.custo and filho.soma_tam_caminhos <= other.soma_tam_caminhos)):
    ...
    def insere_na_frente(self, demanda_caminho, fluxo_aresta, custo, soma_tam_caminhos):
        ...
        if len(grande_frente_pareto) == 0:
            grande_frente_pareto.append(
                cromossomo(tipo='colocar', demanda_caminho=demanda_caminho, fluxo_aresta=fluxo_aresta, custo=custo,
                           soma_tam_caminhos=soma_tam_caminhos))
            menor_custo_frente = custo
            menor_soma_tam_caminho_frente = soma_tam_caminhos
            return

        if (custo < menor_custo_frente and soma_tam_caminhos <= menor_soma_tam_caminho_frente) or (
                custo <= menor_custo_frente and soma_tam_caminhos < menor_soma_tam_caminho_frente):
            frente_pareto = []
        ...
        for i in range(len(grande_frente_pareto)):
            if (custo == grande_frente_pareto[i].custo and soma_tam_caminhos == grande_frente_pareto[
                i].soma_tam_caminhos) or ((grande_frente_pareto[i].custo < custo and grande_frente_pareto[
                i].soma_tam_caminhos <= soma_tam_caminhos) or (
                                                  grande_frente_pareto[i].custo <= custo and grande_frente_pareto[
                                              i].soma_tam_caminhos < soma_tam_caminhos)):
                coloca = False
                break
            if (custo < grande_frente_pareto[i].custo and soma_tam_caminhos <= grande_frente_pareto[
                i].soma_tam_caminhos) or (
                    custo <= grande_frente_pareto[i].custo and soma_tam_caminhos < grande_frente_pareto[
                i].soma_tam_caminhos):
                continue
...
class GA:
    ...
    def __probabilidade_mutacao(self, idx):
        if self.ranks[idx] <= self.rank_medio and self.melhor_rank < self.rank_medio:
            return 0.5 * ((float(self.ranks[idx]) - float(self.melhor_rank)) / (
                        float(self.rank_medio) - float(self.melhor_rank)))
        return 0.5
    ...
    def __Crowding(self):
        ...
        for i in range(len(self.populacao)):
            for j in range(len(self.populacao)):
                distance[i][j] = sqrt(float((self.populacao[i].custo - self.populacao[j].custo) * (
                            self.populacao[i].custo - self.populacao[j].custo) + (
                                                        self.populacao[i].soma_tam_caminhos - self.populacao[
                                                    j].soma_tam_caminhos) * (
                                                        self.populacao[i].soma_tam_caminhos - self.populacao[
                                                    j].soma_tam_caminhos)))
        ...
        for i in range(len(self.populacao)):
            ...
            for j in range(len(self.populacao)):
                ...
                if distance[i][j] > distancia_segundo:
                    ...
                    pos_segundo = j
            self.crowding_distance[i] = abs(
                self.populacao[pos_primeiro].custo - self.populacao[pos_segundo].custo) / max((vet_max[0] - vet_min[0]),
                                                                                              0.1) + abs(
                self.populacao[pos_primeiro].soma_tam_caminhos - self.populacao[pos_segundo].soma_tam_caminhos) / max(
                (vet_max[1] - vet_min[1]), 0.1)
        pass
    ...
    def __torneio_selection(self, vet_idx):
        ...
        if self.ranks[vet_idx[pos1]] < self.ranks[vet_idx[pos]] or (
                self.ranks[vet_idx[pos]] == self.ranks[vet_idx[pos1]] and self.crowding_distance[vet_idx[pos1]] >
                self.crowding_distance[vet_idx[pos]]):
            pos = pos1
        return pos
    ...
    def __plot_frente_de_pareto(self, geracao):
        ...
        soma_tam_caminhos_frente = np.array(
            [grande_frente_pareto[i].soma_tam_caminhos for i in range(len(grande_frente_pareto))])
        ...
        plt.scatter(soma_tam_caminhos, custo, c=np.array(['blue' for i in range(len(self.populacao))]),
                    label='Outras soluções')
        plt.scatter(soma_tam_caminhos_frente, custo_frente,
                    c=np.array(['red' for i in range(len(grande_frente_pareto))]), label='Frente de pareto')
    ...
    def gera_imagem_soluc(self):
        for i in range(len(grande_frente_pareto)):
            ...
            for j in range(len(G.arestas)):
                print('fluxo: ', grande_frente_pareto[i].fluxo_aresta[j], 'capacidade instalada',
